...
class LogisticRegression {
    ...
    this.options = Object.assign(
      {
        ...
        decisionBoundary: 0.5, // default decision boundary
      },
      options
    );
  ...
  gradientDescent(features, labels) {
    ...
    // (Features * Weights)
    // Tensorflow has a sigmoid function to return values between 0 and 1
  ...
  predict(observations) {
    ...
    return (
      this.processFeatures(observations)
        .matMul(this.weights)
        .sigmoid()
        //to round to 0 if value less that our decisionBoundary and to 1 if value greater than decisionBoundary
        .greater(this.options.decisionBoundary)
        // treat values returned by the tensor as float32
        .cast("float32")
    );
  }
  ...
  test(testFeatures, testLabels) {
    // predictions will always return a value that is between 0 and 1 (sigmoid)
